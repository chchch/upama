...
#        $lines = explode($delimiter, $text);
        $lines = preg_split("/([ \n]+)/m", $text, -1, PREG_SPLIT_DELIM_CAPTURE|PREG_SPLIT_NO_EMPTY); // modified for word-level diff
/*        $lines = array($prelines[0]);
        for ($x = 2; $x < count($prelines); $x++) {
            if($x % 2 == 0)
            $lines[] = $prelines[$x-1] . $prelines[$x];
        } */
