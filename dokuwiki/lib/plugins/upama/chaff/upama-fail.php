...
//            return $str;
 //       }
        ...
        foreach ($spaceSplit as $key => $section) {
           ...
           foreach($section as $sec) {
            ...
            if($maintext|$vartext) {
                ...
                $vartexts = false;
                if(strpos($vartext," ") ||
                    ((strlen($vartext)+strlen($maintext)) > (2*$this->affixlemmata))
                ) {
                ...
                $maintext = $this->unfilterText($sharedtext,$counters1,$ignored1,$atlast);
                $finalXml .= $maintext;
    ...
    private function mb_findAffixes($text1,$text2) {
        ...
        if((2*$pre_end) < ($len1+$len2)) { // prefix is less than half the length of the main text or variant text; try computing the suffix
        ...
        if($pre_end >= $post_start &&
                ($pre_end > 5 || ($pre_end > 1 && $arr1[$pre_end-1] == ' ')) 
            ) {
    ...
    private function findAffixes($diffs,$maintext,$vartext) {
        $minlength = 5;
        $pre_dex = 0;
        $suf_dex = 0;
        $prefix = $diffs[0][0] == 0 ? $diffs[0][1] : '';
        $lastel = $diffs[count($diffs)-1];
        $suffix = $lastel[0] == 0 ? $lastel[1] : '';
        $vowels = '/[aāiīeuūoṛṝḷṃḥ\s]/u';
        $consonants = '/[kgcjṭḍtdpbṅñṇnmyrlḻvśṣsh]/u';
        // first half should end in vowel, second half should start with consonant
        if($prefix != '') {
            $pre_arr = preg_split('//u',$prefix,-1,PREG_SPLIT_NO_EMPTY);
            $pre_dex = count($pre_arr) - 1;
            
            while(isset($pre_arr[$pre_dex]) && 
                  !preg_match($vowels,$pre_arr[$pre_dex])) {
                $pre_dex--;
            }
            $prefix = implode('',array_slice($pre_arr,0,$pre_dex+1));
        }
        if($suffix != '') {
            $suf_arr = preg_split('//u',$suffix,-1,PREG_SPLIT_NO_EMPTY);
            
            while(isset($suf_arr[$suf_dex]) && 
                  !preg_match($consonants,$suf_arr[$suf_dex])) {
                $suf_dex++;
            }
            $suffix = implode('',array_slice($suf_arr,$suf_dex));
            $suf_dex = count($suf_arr) - $suf_dex;
        }
        $lengths = array( $suf_dex => 'suffix',
                          $pre_dex => 'prefix',
                          $minlength => 'min',
                          );
        $maxkey = max(array_keys($lengths));
        switch($lengths[$maxkey]) {
            case 'prefix':
                
                $var_arr = preg_split('//u',$vartext,-1,PREG_SPLIT_NO_EMPTY);
                $main_arr = preg_split('//u',$maintext,-1,PREG_SPLIT_NO_EMPTY);
                $var = implode('',array_slice($var_arr,$pre_dex+1));
                $main = implode('',array_slice($main_arr,$pre_dex+1));

                print_r("$prefix :: $var :: $main\n"); 
                return array("var" => $var,
                         "prefix" => $prefix,
                         "main" => $main,
                         );
            ...
            case 'suffix':
                ...
                $var_arr = preg_split('//u',$vartext,-1,PREG_SPLIT_NO_EMPTY);
                $main_arr = preg_split('//u',$maintext,-1,PREG_SPLIT_NO_EMPTY);
                $var = implode('',array_slice($var_arr,0,-$suf_dex));
                $main = implode('',array_slice($main_arr,0,-$suf_dex));

 
                print_r("$suffix :: $var :: $main\n"); 
                return array("var" => $var,
                         "suffix" => $suffix,
                         "main" => $main,
                         );
    ...
    private function unfinished_findAffixes($text1,$text2) {
        $n = 0;
        $char1 = $text1[$n];
        $char2 = $text2[$n];
