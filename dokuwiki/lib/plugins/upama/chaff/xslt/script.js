...
//    upama.script = 'deva';
    ...
    //Hyphenator.run();
     ...
     var apparati = document.getElementsByClassName("variorum");
...
varMouseOver: function() {
    var _this = this;
    var maintext = _this.closest('.apparatus').previousSibling;
//    if(upama.permalit.length == 0 || upama.permalit[0] != _this) {
    if(upama.permalit.length == 0 || upama.permalit[1] != _this) {
        ...
        upama.highlight(hipos,maintext);
    }
},
...
varMouseOut: function() {
    ...
    maintext.innerHTML = maintext.myOldContent; 
...
//        upama.mains[_this.myMainText].myOldContent = upama.mains[_this.myMainText].innerHTML;
        upama.permalit = [maintext,_this,maintext.myOldContent];
        maintext.myOldContent = maintext.innerHTML;
