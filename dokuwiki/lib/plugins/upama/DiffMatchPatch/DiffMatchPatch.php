...
class DiffMatchPatch
{
    ...
    {
/*        $a = $this->diff->getToolkit()->aksarasToChars($text1, $text2);
        $lineText1 = $a[0];
        $lineText2 = $a[1];
        $lineArray = $a[2];

        $diffs = $this->diff->main($lineText1, $lineText2, $checklines)->getChanges();

        $this->diff->getToolkit()->charsToLines($diffs, $lineArray); 
*/
        $diffs = $this->diff->main($text1, $text2, $checklines)->getChanges();
        return $diffs; 
    }
    ...
    public function diff_simpleHtml($diffs)
    {
        $this->diff->setChanges($diffs);
        return $this->diff->simpleHtml();
    }
