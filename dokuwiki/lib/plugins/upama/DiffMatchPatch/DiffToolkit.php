...
//        if ($text1 == '' || $text2 == '' || mb_substr($text1, 0, 1) != mb_substr($text2, 0, 1)) {
    ...
    public function aksarasToChars($text1, $text2)
    {
        // e.g. $lineArray[4] == "Hello\n"
        $lineArray = array();
        // e.g. $lineHash["Hello\n"] == 4
        ...
        $chars1 = $this->aksarasToCharsMunge($text1, $lineArray, $lineHash);
        $chars2 = $this->aksarasToCharsMunge($text2, $lineArray, $lineHash);
    ...
    /**
     * Split a text into an array of strings.  Reduce the texts to a string of hashes where each
     * Unicode character represents one aksara or space.
     * Modifies $lineArray and $lineHash. TODO try to fix it!
     *
     * @param string $text String to encode.
     * @param array  $lineArray
     ...
     * @return string Encoded string.
     */
    protected function aksarasToCharsMunge($text, array &$lineArray, array &$lineHash)
    {
        $chars = '';

        $delimiter = ' ';
        $lines = $this->aksaraSplit($text);
    ...
    protected function aksaraSplit($str) {
