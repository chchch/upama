...
const find = {
    ...
    selection: function() {
        ...
        var target = selrange.startContainer.parentNode.closest(state.mainClass);
        var endtarget = selrange.endContainer.parentNode.closest(state.mainClass);
        ...
        if(target.lastChild === selrange.startContainer && selrange.startOffset === selrange.startContainer.length) {
            let nextindex = [].indexOf.call(state.mains,target) + 1;
            target = state.mains[nextindex];
...
const makePosApp = function(app,msids) {
    if(msids.size === 0) {
        ...
        if(exclude) {
            for(const x of exclude.split(' ')) msids.add(x);
        }
...
const showPopup = function(path,arrays) {
    ...
    const rightClip = target.myData.rightBoundary ? 0 :
                            ...
                            posinfo.sel.stublen;
    const leftClip = (posinfo.pre.endspace || posinfo.sel.startspace) ? 0 :
                            one_word_lemma ?
