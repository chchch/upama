...
//      "cch/ch" => array('(?<!\s)c(?:c| c|)h(?=\S)','(?<!\s)t ś(?=\S)'),
        "cch/ch" => array('c\s?ch','t\sś', "replace_with" => 'ch'),
...
//       "nasals" => array('ṅ(?=[kg])|ñ(?=[cj])|ṇ(?=[ṭḍ])|m(?=[pbd])|n(?=[tdn])', "replace_with" => 'ṃ'),
        ...
        "sya,tra,ma before iti" => array('(?<=sy|tr|m)a\si(?=t[iīy])', "replace_with" => 'e'),

// most of these iti rules can be applied only to printed editions
        "e/a + iti" => array('e(?=\sit[iīy])',"replace_with" => 'a'),
        "i iti/īti" => array('i\si(?=t[iīy])', "replace_with" => 'ī'),
